...
export default {
  ...
  computed: {
    ...
    */
    uname () {
      try {
        let user = this.players.find(p => p.id === this.myHand.uId)
  ...
  methods: {
    sit () {
      let seat = { action: 'sit', uId: this.myHand.uId, sId: this.myHand.seatX }
      this.$emit('onHand', seat)
    },
    play (n) {
      if (this.isTurn()) {
        let card = this.getN(n) + this.getSuit(n)
